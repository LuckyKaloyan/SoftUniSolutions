    ...
    public static void main(String[] args) {
        ...
        while (playerPoints > 0 && hsPoints > 0) {
            ...
            if (hsChamber[startPlRow][startPlCol]) {
                if (startPlRow - 1 >= 0 && !hsChamber[startPlRow - 1][startPlCol]) {
                    startPlRow--;
                } else if (startPlCol + 1 < hsChamber[startPlRow].length && !hsChamber[startPlRow][startPlCol + 1]) {
                    startPlCol++;
                } else if (startPlRow + 1 < hsChamber.length && !hsChamber[startPlRow + 1][startPlCol]) {
                    startPlRow++;
                } else if (startPlCol - 1 >= 0 && !hsChamber[startPlRow][startPlCol - 1]) {
                    startPlCol--;
                }
                if (hsChamber[startPlRow][startPlCol]) {
                    switch (spell) {
